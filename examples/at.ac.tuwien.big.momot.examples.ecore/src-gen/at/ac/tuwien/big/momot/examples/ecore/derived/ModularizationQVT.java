...
public class ModularizationQVT {
  ...
  protected static final String INITIAL_MODEL = "input/QVT_module.xmi";
  ...
  public class ModularizationQVTFitnessFunction extends EGraphMultiDimensionalFitnessFunction {
    ...
    protected void preprocessEvaluation(final TransformationSolution solution) {
      final Language root = MomotUtil.<Language>getRoot(solution.execute(), Language.class);
      solution.setAttribute(ModularizationQVT.attribute, MetricsCalculator.calculate(root));
    }
  }
  ...
  protected double _createObjectiveHelper_2(final TransformationSolution solution, final EGraph graph, final EObject root) {
    final Function1<at.ac.tuwien.big.momot.examples.ecore.modularization.Module, Boolean> _function = (at.ac.tuwien.big.momot.examples.ecore.modularization.Module m) -> {
      boolean _isEmpty = m.getEntities().isEmpty();
      return Boolean.valueOf((!_isEmpty));
    };
    return IterableExtensions.size(IterableExtensions.<at.ac.tuwien.big.momot.examples.ecore.modularization.Module>filter(((Language) root).getModules(), _function));
  }
  ...
  protected double _createObjectiveHelper_3(final TransformationSolution solution, final EGraph graph, final EObject root) {
    int _xblockexpression = (int) 0;
    {
      ...
      final Function1<at.ac.tuwien.big.momot.examples.ecore.modularization.Module, Integer> _function_1 = (at.ac.tuwien.big.momot.examples.ecore.modularization.Module m) -> {
        return Integer.valueOf(m.getEntities().size());
      };
      final Iterable<Integer> sizes = IterableExtensions.<at.ac.tuwien.big.momot.examples.ecore.modularization.Module, Integer>map(IterableExtensions.<at.ac.tuwien.big.momot.examples.ecore.modularization.Module>filter(((Language) root).getModules(), _function), _function_1);
