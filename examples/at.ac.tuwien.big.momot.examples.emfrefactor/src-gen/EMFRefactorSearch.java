...
public class EMFRefactorSearch {
  protected static final String INITIAL_MODEL = "model/input/metamodel.ecore";

  protected static final int SOLUTION_LENGTH = 5;
  ...
  protected double _createObjectiveHelper_1(final TransformationSolution solution, final EGraph graph, final EObject root) {
    ...
    final List<EObject> eClasses = graph.getDomain(EcorePackage.Literals.ECLASS.eClass(), true);
  ...
  protected boolean _createEqualityHelperHelper_(final EObject left, final EObject right) {
    ...
    return new DefaultEObjectEqualityHelper().equals(left, right);
  }
